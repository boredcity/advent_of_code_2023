namespace AdventOfCode {
    class Day11 : IDay {
        public string Solve1() => GetDistancesSum(1).ToString();
        public string Solve2() => GetDistancesSum(1000000m - 1).ToString();

        private decimal GetDistancesSum(decimal emptyGrowth) {
            ...
            for (var rI = 0; rI < map.Count; rI++) {
                var isEmptyRow = true;
                for (var cI = 0; cI < map.Count; cI++) if (map[rI][cI] == '#') {
                    isEmptyRow = false;
                    galaxies.Add((rI, cI));
                }
                if ((rI + 1) < emptyPrevRows.Length) emptyPrevRows[rI + 1] = emptyPrevRows[rI] + (isEmptyRow ? emptyGrowth : 0);
            }

            for (var cI = 0; cI < map.Count; cI++) {
                var isEmptyColumn = true;
                for (var rI = 0; rI < map.Count; rI++) if (map[rI][cI] == '#') isEmptyColumn = false;
                if ((cI + 1) < emptyPrevColumns.Length) emptyPrevColumns[cI + 1] = emptyPrevColumns[cI] + (isEmptyColumn ? emptyGrowth : 0);
            }
